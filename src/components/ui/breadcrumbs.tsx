  ...
  ({ className, truncate = false, title, ...props }, ref) => {
    return (
      <span
        aria-current="page"
        className={cn(
          "inline-flex min-w-0 items-center font-normal text-inherit",
          truncate && breadcrumbTruncateClassName,
          className
        )}
...
>(({ children, className, ...props }, ref) => {
  return (
    <motion.li
      animate="animate"
      aria-hidden="true"
      className={cn("inline-flex items-center opacity-60 [&>svg]:size-3.5 [&>svg]:translate-y-[1px]", className)}
  ...
  return (
    <div className={cn("relative inline-flex", className)} ref={rootRef}>
      <button
        ...
        className={cn(
          breadcrumbLinkClassName,
          "flex size-5 items-center justify-center text-muted-foreground hover:text-[#6495ed] [&>svg]:size-4"
        )}
