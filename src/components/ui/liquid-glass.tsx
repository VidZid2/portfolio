...
export function generateSmoothConvexMap(width: number, height: number, renderMode = "svg") {
...
export function analyzeRefraction(data: Uint8ClampedArray, width: number, height: number, lightAz: number) {
  ...
  ({ bgImage, fallbackMode = "webgl", className, children, ...props }, ref) => {
    ...
    const registerButton = React.useCallback((id: string, element: HTMLButtonElement) => {
      ...
      if (mapData) {
        ...
        if (mode === "svg") {
          feImage0Ref.current?.setAttribute("href", mapData.url);
          feImage1Ref.current?.setAttribute("href", mapData.url);
        } else if (mode === "webgl") {
          glSetDisplacement(mapData.url);
        }
      }
    }, [mode, glSetDisplacement]);
    ...
    React.useEffect(() => {
      ...
      const isSafariMac = typeof navigator !== "undefined" && /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
      let detectedMode: "svg" | "webgl" | "blur" = "svg";
      if (isIOS || isSafariMac) detectedMode = fallbackMode === "webgl" ? "webgl" : "blur";
      
      setMode(detectedMode);

      if (detectedMode === "webgl" && canvas && bgImage) {
      ...
      const loop = () => {
        ...
        if (pRect.width > 0 && pRect.height > 0) {
          ...
          if (key !== lastKeyRef.current && lastMapRef.current) {
            lastKeyRef.current = key;
            const analysis = analyzeRefraction(
              lastMapRef.current.data,
              lastMapRef.current.width,
              lastMapRef.current.height,
              lightAz
            );
            if (analysis) {
    ...
    }, [initWebGL, glSetDisplacement, fallbackToCSSBlur, fallbackMode, filterId0, filterId1, bgImage]);
...
export const LiquidGlassButton = React.forwardRef<HTMLButtonElement, LiquidGlassButtonProps>(
  ({ className, children, ...props }, ref) => {
    ...
    const rawFilterId = React.useId().replace(/:/g, "-");
    const standaloneFilterId = `glass-filter-${rawFilterId}`;
    const [standaloneMapUrl, setStandaloneMapUrl] = React.useState<string>("");
    ...
    // Generate local displacement normal map when used standalone
    React.useEffect(() => {
      if (context) return;
      ...
      const map = generateSmoothConvexMap(w, h, "svg");
      if (map) {
        setStandaloneMapUrl(map.url);
        const analysis = analyzeRefraction(map.data, map.width, map.height, -Math.PI / 2);
        if (analysis) {
          const intensity = 0.4 + analysis.magnitude * 0.6;
          const cosVal = -Math.cos(analysis.domAngle) * intensity;
          const sinVal = -Math.sin(analysis.domAngle) * intensity;
    ...
    return (
      <button
        ...
        {...props}
      >
        {/* Standalone SVG displacement filter definition */}
        {standaloneMapUrl && !context && (
          <svg className="absolute w-0 h-0 overflow-hidden pointer-events-none" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <filter id={standaloneFilterId} x="0" y="0" width="100%" height="100%" filterUnits="userSpaceOnUse" primitiveUnits="userSpaceOnUse" colorInterpolationFilters="sRGB">
                <feImage href={standaloneMapUrl} x="0" y="0" width="100%" height="100%" result="lens" preserveAspectRatio="none" />
                <feFlood floodColor="rgb(128,128,128)" result="neutral" />
                <feComposite in="lens" in2="neutral" operator="over" result="dispMap" />
                <feDisplacementMap in="SourceGraphic" in2="dispMap" scale={DISP_SCALE.toString()} xChannelSelector="R" yChannelSelector="G" />
              </filter>
        ...
        <span
          ...
          style={{
            ...
            backdropFilter: standaloneMapUrl && !context
              ? `url(#${standaloneFilterId}) blur(2px) saturate(180%) brightness(1.05)`
              : renderMode === "webgl"
              ? "none"
              : "blur(2px) saturate(180%) brightness(1.05)",
            WebkitBackdropFilter: standaloneMapUrl && !context
              ? `url(#${standaloneFilterId}) blur(1px) saturate(180%) brightness(1.05)`
              : renderMode === "webgl"
              ? "none"
        ...
        <span className="relative z-20 text-sm font-semibold tracking-wide text-black/85 dark:text-white/90 select-none pointer-events-none flex items-center justify-center gap-2">
          {children}
        </span>
      </button>
    );
