...
export const createSmokeRenderer = (
  ...
  (
    ...
    return (
      <button
        ...
        ref={ref}
        type={type}
        className={`relative overflow-hidden inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-medium text-white transition-colors ${className}`.trim()}
      >
        <canvas ref={canvasRef} className="absolute inset-0 size-full pointer-events-none" aria-hidden="true" />
        <span className="relative z-10 flex items-center gap-2">
          <span>{children}</span>
          {status ? <span className="text-xs opacity-75">{status}</span> : null}
        </span>
      </button>
    );
  },
