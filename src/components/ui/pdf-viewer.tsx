...
import DynamicIslandNotification from "./pdf-notification"
...
function PDFViewerZoomDropdownControl({
  currentZoomLevel,
  controlsDisabled,
  onZoomChange,
}: {
...
function PDFViewerSearchControl({
  ...
  if (disabled) {
    return (
      <ToolbarTooltip label="Search not available">
        <Button
          type="button"
          variant="ghost"
          size="icon-sm"
          ...
          disabled
          className="opacity-40 cursor-not-allowed text-zinc-400 dark:text-zinc-600"
        >
...
function PDFViewerScrollAreaViewport({
  ...
  React.useEffect(() => {
    ...
    const handleNativeWheel = (e: WheelEvent) => {
      if (e.ctrlKey || e.metaKey) {
        // Prevent standard browser pinch-zoom/page zoom
        e.preventDefault()
        e.stopPropagation()
  ...
  }, [zoom, zoomState.currentZoomLevel, viewportRef])

  // Automatically center view horizontally when document loads, zoom changes, or window resizes
  React.useEffect(() => {
    const el = viewportRef.current
    if (!el) return
    ...
    const t1 = setTimeout(centerHorizontal, 50)
    const t2 = setTimeout(centerHorizontal, 150)
    const t3 = setTimeout(centerHorizontal, 350)
    const t4 = setTimeout(centerHorizontal, 600)
    return () => {
      clearTimeout(t1)
      clearTimeout(t2)
      clearTimeout(t3)
  ...
  }, [documentId, zoomState.currentZoomLevel, viewportRef])
...
function PDFViewerInner({
      ...
      return (
        ...
        >
          <PagePointerProvider
            ...
            className={cn(
              "relative w-full h-full border border-transparent bg-transparent shadow-xs select-none selection:bg-transparent selection:text-inherit",
  ...
  return (
    ...
    >
      {showToolbar ? (
        <div className="relative flex min-h-12 flex-nowrap items-center justify-between gap-4 border-b bg-background px-3 py-2 overflow-x-auto scrollbar-hide shrink-0 z-30">
          <div className="relative flex shrink-0 items-center gap-2">
            ...
            {title && (
              <div
                className={cn(
                  "absolute left-0 top-1/2 -translate-y-1/2 flex items-center gap-2 transition-opacity duration-200 pointer-events-none",
                  activeViewMode === "image"
                    ? "opacity-100 pointer-events-auto"
                    : "opacity-0"
                )}
            ...
            <div className="flex shrink-0 items-center justify-end gap-1">
              ...
              >
                ...
                <div className="flex flex-none items-center gap-1">
                  ...
                  <PDFViewerZoomDropdownControl
                    currentZoomLevel={currentZoomLevel}
                    controlsDisabled={controlsDisabled}
                    onZoomChange={(nextZoom) => zoom?.requestZoom(nextZoom)}
                  />
              ...
              {toolbarActions ? (
                <>
                  <div
                    className={cn(
                      "border-l border-neutral-300 dark:border-neutral-700 h-5 self-center mx-1 transition-opacity duration-200",
                      activeViewMode === "image"
                        ? "opacity-0 pointer-events-none"
                        : "opacity-100"
                    )}
      ...
      {photoContent ? (
        <div className="relative flex min-h-0 flex-1 overflow-hidden bg-muted/30 w-full">
          <motion.div
            className="flex w-[200%] min-w-[200%] shrink-0 h-full transform-gpu will-change-transform"
            initial={false}
            animate={{
              x: activeViewMode === "pdf" ? "0%" : "-50%",
            }}
            transition={{
              type: "spring",
              stiffness: 340,
          ...
          >
            {/* Slide 1: PDF Viewer Shell (100% of modal viewport width) */}
            <div
              ref={viewerShellRef}
              className="relative flex min-h-0 w-1/2 min-w-[50%] max-w-[50%] shrink-0 h-full overflow-hidden bg-muted/30"
            >
              {isLoading ? (
            ...
            {/* Slide 2: Photo Content (100% of modal viewport width) */}
            <div className="relative flex min-h-0 w-1/2 min-w-[50%] max-w-[50%] shrink-0 h-full overflow-hidden bg-neutral-100/60 dark:bg-neutral-950/60 items-center justify-center p-2 sm:p-6">
              {photoContent}
            </div>
          </motion.div>
        </div>
      ...
      <EmbedPDF engine={engine} plugins={plugins}>
        ...
        >
          <ul className="list-disc pl-4 space-y-1.5">
            <li className="hidden sm:list-item"><strong className="text-neutral-900 dark:text-neutral-100">Desktop:</strong> Use <kbd className="px-1.5 py-0.5 rounded-md bg-neutral-100 dark:bg-neutral-800 border dark:border-neutral-700 text-xs font-mono">Middle Click + Drag</kbd> to pan, and <kbd className="px-1.5 py-0.5 rounded-md bg-neutral-100 dark:bg-neutral-800 border dark:border-neutral-700 text-xs font-mono">Ctrl + Scroll</kbd> to zoom in or out.</li>
            <li className="sm:hidden"><strong className="text-neutral-900 dark:text-neutral-100">Mobile:</strong> Use standard <kbd className="px-1.5 py-0.5 rounded-md bg-neutral-100 dark:bg-neutral-800 border dark:border-neutral-700 text-xs font-mono">Pinch</kbd> to zoom, and a <kbd className="px-1.5 py-0.5 rounded-md bg-neutral-100 dark:bg-neutral-800 border dark:border-neutral-700 text-xs font-mono">Single-finger drag</kbd> to pan.</li>
          </ul>
        </DynamicIslandNotification>
      </EmbedPDF>
    )
