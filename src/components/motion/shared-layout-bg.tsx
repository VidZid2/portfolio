    ...
    const renderedChildren = Children.toArray(children)
      ...
      .map((child, index) => {
        const el = child as ReactElement<{
          ...
          onMouseEnter?: () => void;
          children?: ReactNode;
          "data-pill-class"?: string;
          "data-non-interactive"?: boolean;
        }>;
        if (el.props["data-non-interactive"]) {
          return el;
        }
        const childKey = el.key ? String(el.key) : `item-${index}`;
        const itemPillClass = el.props["data-pill-class"] || pillClassName;
        ...
        return cloneElement(
          ...
          <>
                ...
                >
                  {activeId === childKey ? (
                    <motion.div
                      ...
                      className={cn(
                        "pointer-events-none h-full w-full rounded-[6px] bg-neutral-300/60 dark:bg-neutral-700/60",
                        itemPillClass,
                      )}
                    />
                  ) : null}
            ...
            <div className="relative z-10 w-full flex items-center justify-between">{el.props.children}</div>
          </>,
        );
      });
