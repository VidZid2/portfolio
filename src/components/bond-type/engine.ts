  ...
  private layout() {
    ...
    LINES.forEach((word, li) => {
      ...
      for (let i = 0; i < word.length; i++) {
        ...
        // BONDS ONLY CONNECT LETTERS WITHIN A WORD
        if (i > 0 && word[i] !== " " && word[i - 1] !== " ") {
          this.pairs.push([start + i - 1, start + i]);
        }
      }
      ...
      POSES.forEach((pose) => {
        ...
        ls.forEach((l, i) => {
          if (i > 0) x += (gaps[i - 1] || gaps[gaps.length - 1] || 0.2) * H;
          l.px.push(x - cx(l));
          const dyVal = pose.dy[li] ? (pose.dy[li][i] ?? 0) : 0;
          l.py.push(dyVal * H);
        });
      });
    });
