...
export class DotCut {
  ...
  private isDark = true;
  private palettes: [string, string][] = DARK_PALETTES;
  private symbol = "JD";
  ...
  private fontFamily = "var(--font-geist-sans), -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";

  constructor(host: HTMLElement, isDark = true, symbol = "JD", cols?: number, fontFamily?: string) {
    this.host = host;
    this.isDark = isDark;
    this.symbol = symbol;
    this.palettes = isDark ? DARK_PALETTES : LIGHT_PALETTES;
    if (cols) this.params.cols = cols;
    if (fontFamily) this.fontFamily = fontFamily;

    // Apply symbol to the text scene
    SCENES[0].value = symbol;
  ...
  setTheme(isDark: boolean) {
    ...
    this.isDark = isDark;
    this.prevPalette = this.sceneIdx;
    this.palettes = isDark ? DARK_PALETTES : LIGHT_PALETTES;
    this.paletteMix = 0;
