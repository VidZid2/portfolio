...
export const SCENES: Scene[] = [
  { kind: "text", value: "JD", transition: "wipe", palette: 0, style: "drift" },
...
// Cornflower Blue palettes for Dark Mode: [circleColor, backgroundColor]
// Circle: Cornflower Blue / bright tints, Background: Dark charcoal / black (shows through the negative space hole)
export const DARK_PALETTES: [string, string][] = [
  ["#6495ED", "#09090b"], // Cornflower Blue mesh on Dark Black
  ["#8aa9ff", "#0b0f19"], // Vibrant soft cornflower on Midnight
  ["#ffffff", "#121827"], // Pure White on Dark Slate
  ["#6495ED", "#0d1117"], // Cornflower on Charcoal
  ["#a5b4fc", "#080c14"], // Light cornflower on Deep Black
  ["#7ba7f5", "#09090b"], // Classic Cornflower on Dark Black
];

// Cornflower Blue palettes for Light Mode: [circleColor, backgroundColor]
// Circle: Cornflower Blue / deep shades, Background: White / clean light paper
export const LIGHT_PALETTES: [string, string][] = [
  ["#6495ED", "#ffffff"], // Cornflower Blue mesh on Pure White
  ["#2563eb", "#f4f8ff"], // Royal Blue on Light Tint
  ["#1e293b", "#eff5ff"], // Dark Slate mesh on Soft Cornflower
  ["#6495ED", "#f8fafc"], // Cornflower on Off-White
  ["#3b82f6", "#ffffff"], // Vivid Blue on Pure White
  ["#1d4ed8", "#f0f6ff"], // Deep Blue on Soft Tint
];

export const PALETTES: [string, string][] = DARK_PALETTES;
...
export function rasterize(
  ...
  const text = (scene.value || "JD").trim();
  if (!text) return out;
  ...
  // Sizing: for 2-letter monograms (like "JD") allow up to 64% width; for 1 letter allow 36%
  const maxW = text.length > 1 ? cols * 0.64 : cols * 0.36;
