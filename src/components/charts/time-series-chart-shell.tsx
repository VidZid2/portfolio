...
function resolveTimeSeriesYDomain(
  ...
  if (yScaleDomainMax != null && yScaleDomainMax > 0) {
    return [-yScaleDomainMax * 0.2, yScaleDomainMax * 1.15];
  }

  const { minValue, maxValue } = collectNumericExtents(data, dataKeys);

  const effectiveMax = maxValue <= 0 ? 5 : maxValue;
  const bottomPadding = effectiveMax * 0.25;
  const topPadding = effectiveMax * 0.2;

  return [Math.min(0, minValue) - bottomPadding, Math.max(effectiveMax, maxValue) + topPadding];
}
