...
const ChartTooltipInner = memo(function ChartTooltipInner({
  ...
  const tooltipContent = (
    <>
      {/* Crosshair indicator - rendered as SVG overlay with smooth fade */}
      {showCrosshair && (
        <svg
          aria-hidden="true"
      ...
      {/* Dots on bars/lines - show for vertical charts only with smooth fade */}
      {showDots && !isHorizontal && (
        <svg
          aria-hidden="true"
          className="pointer-events-none absolute inset-0"
          height="100%"
...
function DatePillTracker(props: DatePillTrackerProps) {
  ...
  return (
    <AnimatePresence>
      {props.visible && props.labels.length > 0 && (
        <DatePillTrackerInner {...props} />
      )}
    </AnimatePresence>
  );
}
