...
const ReasoningSteps = forwardRef<HTMLDivElement, ReasoningStepsProps>(
  function ReasoningSteps(
    ...
    const wasActiveRef = useRef(hasActive);
    useEffect(() => {
      if (wasActiveRef.current && !hasActive && open) {
        // Auto-close after a short delay when thinking finishes
    ...
    }, [hasActive, open, handleOpenChange]);
...
function ReasoningStepsTrigger({
  ...
  return (
    <CollapsiblePrimitive.Trigger
      className={cn(
        "flex w-full items-center gap-3 py-2 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
        className
      )}
