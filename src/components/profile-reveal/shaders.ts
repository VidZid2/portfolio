...
void main() {
  ...
  float faded = min(min(c1, c2), min(c3, c4)) * (1.0 - uDt * 0.45);

  float d = distance(uv, uPointer);
  d += cnoise(vec3(uv * 2.5, uTime * 0.8)) * 0.45 * uSize;
...
vec2 cover(vec2 uv) { return (uv - 0.5) * uCover / 1.05 + 0.5 + uParallax; }
...
float cellOn(float v) { return step(0.35, v); }

float cellPop(float v) {
  return smoothstep(0.35, 0.55, v) * (1.0 - smoothstep(0.55, 0.85, v));
}
...
void main() {
  vec2 uvA = cover((vUv - 0.5) / uZoomA + 0.5);
  vec3 style = texture2D(uStyleA, uvA).rgb;

  vec2 realUV = cover((vUv - 0.5) / uRealZoom + 0.5) + uRealShift;
  vec3 realCrisp = texture2D(uReal, realUV).rgb;

  // Reveal Grid using Rounded-Box SDF Merging
  ...
  // Blend from Front Anime Avatar to Crisp Real Face Photo
  vec3 outc = mix(style, realCrisp, m);

  // Subtle block bevel border on reveal edge
  float e = 0.03;
  ...
  outc *= 1.0 - band * 0.22 * clamp(-lightBias, 0.0, 1.0);
  outc += band * 0.15 * clamp(lightBias, 0.0, 1.0) * m;
