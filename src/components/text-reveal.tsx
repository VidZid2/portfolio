...
// Extracted from typical ease definitions
        ...
        return (
          <span key={lineKey} className="block">
            {units.map((unit, i) => {
              ...
              const finalState = { y: 0, opacity: 1, filter: "blur(0px)" };
              const initial = skipAnimation 
                ? finalState 
                : (reduce ? { opacity: 0 } : { y: yOffset, opacity: 0, filter: `blur(${blur}px)` });
              const animate = skipAnimation 
                ? finalState 
                : (shouldAnimate ? (reduce ? { opacity: 1 } : finalState) : initial);
              const transition: Transition = (skipAnimation || reduce)
                ? { opacity: { duration: skipAnimation ? 0 : 0.25, ease: EASE_OUT, delay: skipAnimation ? 0 : d * 0.3 } }
                : {
