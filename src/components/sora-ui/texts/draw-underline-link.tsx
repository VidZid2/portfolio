...
const drawUnderlineLinkVariants = cva(
  "relative inline-block w-fit cursor-pointer no-underline group select-none align-baseline",
  {
    variants: {
      variant: {
        default: "",
...
const drawUnderlineLinkTextVariants = cva("inline leading-normal");

const drawUnderlineLinkBoxVariants = cva(
  "absolute left-0 right-0 top-full h-[7px] sm:h-[8px] w-full overflow-visible pointer-events-none -mt-0.5"
);
...
export interface DrawUnderlineLinkProps
  ...
  /** Plain-text label or React child */
...
function DrawUnderlineLink({
  ...
  const text = (typeof children === "string" ? children : label) ?? children ?? "";
  ...
  const currentPath = paths[activePathIndex ?? 0] ?? paths[0];

  // Mobile / Tablet: always show the cornflower blue hand-drawn stroke automatically
  ...
  return (
    ...
    >
      ...
      >
        {isMobileOrTablet ? (
          /* Automatically visible on mobile/tablet */
          <StaticUnderlinePath
            path={currentPath}
          ...
          /* Animated Cornflower Blue Hand-Drawn Stroke on hover */
          activePathIndex !== null && phase !== "idle" ? (
            <svg
              aria-hidden="true"
              className="absolute inset-0 h-full w-full overflow-visible pointer-events-none"
              preserveAspectRatio="none"
