...
export interface StageConfig {
  ...
  curvature: number;
  zoom: number;
  /** IntersectionObserver margin for the play band. */
  rootMargin?: string;
...
export function useSwirlStage(
  ...
  const { isLowEndDevice } = useHardware();
  ...
  useEffect(() => {
    ...
    if (typeof IntersectionObserver !== "undefined") {
      io = new IntersectionObserver(
        ...
        // or use the custom rootMargin if provided in the config.
        { threshold: 0, rootMargin: cfgRef.current?.rootMargin || "-35% 0px -35% 0px" },
      );
      io.observe(canvas);
